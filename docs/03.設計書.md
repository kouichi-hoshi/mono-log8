...
- ごみ箱表示（`view=trash`）時は UI/UX を単純に保つため **タグ/お気に入り絞り込みは無効（UIは非表示）** とし、`status=trashed` の一覧を表示する（URL上の値は保持）
...
- URL に含まれる `tagId` がタグクラウドに存在しない場合（削除/未同期等）でも、ユーザーが解除できるよう「不明なタグ」として選択状態に表示し、`tags` から除外できる導線を提供する
...
| 観点 | ルール |
| --- | --- |
| 大小文字 | 区別する（案A）。例: `React` と `react` は別タグ |
...
| --- | --- | --- | --- | --- | --- |
| ログイン（OAuth）失敗/キャンセル | - | `toast.error` | 「認証エラーが発生しました」 | ログインモーダルは閉じない（維持）。ユーザーが再試行できる状態を保つ | モーダル内の「Googleでログイン」で再試行 |
...
| バリデーション（入力不備） | 422 | （sonner ではなく Alert） | 「入力内容に不備があります」/「内容を入力してください」 | Alert を表示し、入力状態は維持する | 入力修正→再実行 |
