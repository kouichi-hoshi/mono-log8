...
- 本リポジトリは **pnpm** を利用する（`docs/03.v2/01_基盤-方針.md` を参照）
...
- スタブ有効化（例）
	...
	- `USE_STUB_POSTS=true`
- 注意
	- 本番環境（`NODE_ENV=production`）では上記が設定されていても **スタブが有効にならない**ガードを前提とする（詳細は `docs/03.v2/20_認証-認可-スタブ.md` を参照）
...
- `.env` / `.env.local` の方針は `docs/03.v2/02_設定-環境変数-PWA.md` を参照
- DB接続を行う場合のキー例
	- `DATABASE_URL`
